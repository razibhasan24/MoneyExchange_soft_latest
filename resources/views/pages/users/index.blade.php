    ...
    <div class="card bg-info mb-3 p-4">
        <div class="row">
            <div class="col-12 d-flex justify-content-between align-item-center ">
                <h3 class=" card-title text-white d-flex align-items-center  m-0">User List</h3>
                <a href="{{ route('users.create') }}" class="btn btn-light btn-sm" title="Create New Product">
    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Username</th><th>Password hash</th><th>Full name</th><th>Role</th><th>Created at</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($users as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->username }}</td><td>{{ $item->password_hash }}</td><td>{{ $item->full_name }}</td><td>{{ $item->role }}</td><td>{{ $item->created_at }}</td><td style="min-width:220px">
